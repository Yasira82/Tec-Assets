// NOTE: this file deliberately declares NO environment override. This
// project's global setupFiles touches `window`, so pinning the node
// environment makes the SETUP fail before a single assertion runs — and
// nothing here needs node: it reads two files and calls a pure function.
//
// Do not name the directive in a comment either. Vitest scans the file
// TEXT for it, so even a comment explaining why it is absent turns it on.
//
...
describe('the route and the fallback both obey C-123', () => {
  // Resolved from the project root rather than `import.meta.url`: happy-dom
  // supplies its own URL implementation, which rejects the module URL this
  // file is loaded with.
  const src = join(process.cwd(), 'src/app/api/auth/sso-callback/route.ts');
  ...
  it('the document.cookie fallback does NOT set Partitioned — and that is deliberate', () => {
    // Reversed after Mainnet auth turned intermittent. The server response
    // above DOES set it (C-123 LAW 3); this fallback's value was the
    // unpartitioned duplicate beside it, and removing that left no cookie at
    // all in any context where the partitioned copy is not sent.
    //
    // Pinned so the "obvious consistency fix" is not reapplied without
    // reading why it was undone.
    const s = read();
